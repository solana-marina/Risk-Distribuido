...
class RiskClientApp:
    ...
    def _handle_event(self, event: pygame.event.Event) -> None:
        ...
        if self.state == "menu":
            ...
            if self.rules_visible or self.history_visible:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    ...
                    for button in reversed(self.game_buttons):
                        if button.action[0] in {"close_rules", "rules_page", "close_history", "history_page"} and button.enabled and button.rect.collidepoint(mouse_pos):
                            self._activate(button.action)
                            return
                return
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and mouse_pos:
    ...
    def _activate(self, action: tuple[str, object | None]) -> None:
        ...
        if kind == "host":
            ...
            self.rules_visible = False
            self.history_page = 0
        elif kind == "close_history":
            self.history_visible = False
        elif kind == "history_page":
            self.history_page = max(0, self.history_page + int(value or 0))
    ...
    def _draw_game(self) -> None:
        ...
        self._draw_top_actions(board_rect)
        if self.rules_visible:
            self._draw_rules_modal()
        if self.history_visible:
            self._draw_history_modal(snapshot)
    ...
    def _draw_left_sidebar(self, snapshot: dict[str, object]) -> None:
        ...
        for index, (continent, entry_color) in enumerate(entries):
            row = pygame.Rect(legend_box.x + 8, legend_box.y + 34 + index * 33, legend_box.width - 16, 28)
            ...
            for line_index, line in enumerate(wrap_text(label, self.font_tiny, row.width - 30, max_lines=2)):
                self.screen.blit(self.font_tiny.render(line, True, TEXT), (row.x + 24, row.y + 3 + line_index * 11))
    ...
    def _draw_prompt_and_buttons(self, snapshot: dict[str, object], x: int, y: int, width: int) -> None:
        ...
        prompt = snapshot.get("pending_prompt") or {}
        if isinstance(prompt, dict) and prompt.get("type") == "capture_move":
            hint = fit_text(f"Mover tropas: {self.move_count}", self.font_small, width - 20)
    ...
    def _draw_top_actions(self, board_rect: pygame.Rect) -> None:
        button_rect = pygame.Rect(0, 28, 132, 34)
        button_rect.centerx = board_rect.centerx
        rules_button = Button(button_rect, "Regras", ("show_rules", None))
    ...
    def _draw_history_modal(self, snapshot: dict[str, object]) -> None:
        overlay = pygame.Surface(WINDOW_SIZE, pygame.SRCALPHA)
        overlay.fill((0, 0, 0, 170))
        self.screen.blit(overlay, (0, 0))

        modal = pygame.Rect(150, 54, WINDOW_SIZE[0] - 300, WINDOW_SIZE[1] - 108)
        self._draw_panel(modal, PANEL)
        self.screen.blit(self.font_big.render("Histórico Completo", True, TEXT), (modal.x + 24, modal.y + 18))
        subtitle = "Todas as ações registradas pelo servidor autoritativo nesta partida."
        self.screen.blit(self.font_small.render(subtitle, True, SUBTLE), (modal.x + 24, modal.y + 54))

        close_button = Button(pygame.Rect(modal.right - 120, modal.y + 20, 96, 34), "Fechar", ("close_history", None))
        self.game_buttons.append(close_button)
        close_button.draw(self.screen, self.font_small)

        content = pygame.Rect(modal.x + 28, modal.y + 98, modal.width - 56, modal.height - 176)
        pygame.draw.rect(self.screen, PANEL_ALT, content, border_radius=12)
        pygame.draw.rect(self.screen, (60, 67, 78), content, width=1, border_radius=12)

        wrapped_lines = self._wrapped_history_lines(snapshot, content.width - 24)
        line_height = 18
        lines_per_page = max(1, (content.height - 22) // line_height)
        page_count = max(1, (len(wrapped_lines) + lines_per_page - 1) // lines_per_page)
        self.history_page = max(0, min(self.history_page, page_count - 1))
        start = self.history_page * lines_per_page
        visible_lines = wrapped_lines[start : start + lines_per_page]
        for index, line in enumerate(visible_lines):
            color = SUBTLE if line else (90, 96, 108)
            self.screen.blit(self.font_small.render(line, True, color), (content.x + 12, content.y + 12 + index * line_height))

        page_label = f"Página {self.history_page + 1} de {page_count}"
        self.screen.blit(self.font_small.render(page_label, True, SUBTLE), (modal.x + 28, modal.bottom - 54))
        prev_button = Button(
            pygame.Rect(modal.right - 310, modal.bottom - 62, 90, 36),
            "Anterior",
            ("history_page", -1),
        ...
        next_button = Button(
            ...
            "Próxima",
            ("history_page", 1),
            self.history_page < page_count - 1,
        )
        self.game_buttons.extend([prev_button, next_button])
        prev_button.draw(self.screen, self.font_small)
        next_button.draw(self.screen, self.font_small)
    ...
    def _draw_log(self, snapshot: dict[str, object], x: int, y: int, width: int, height: int) -> None:
        ...
        self._draw_panel(box, PANEL_ALT)
        header_bottom = self._draw_section_header("Histórico de ações", x + 10, y + 8, width - 20)
        history_button = Button(
            pygame.Rect(x + 10, header_bottom + 4, width - 20, 30),
            "Ver histórico completo",
            ("show_history", None),
        )
        self.game_buttons.append(history_button)
        history_button.draw(self.screen, self.font_tiny)
        battle = snapshot.get("last_battle") or {}
        cursor_y = history_button.rect.bottom + 10
        if isinstance(battle, dict) and battle:
            cursor_y = self._draw_battle_dice(battle, x + 10, cursor_y, width - 20)
        self._draw_recent_log_entries(snapshot, x + 10, cursor_y, width - 20, box.bottom - 10)

    def _draw_recent_log_entries(
        self,
        ...
        entries = self._history_entries(snapshot)
        line_height = 14
        blocks: list[list[str]] = []
        used_height = 0
        for entry in reversed(entries):
            lines = wrap_text(str(entry), self.font_tiny, width, max_lines=3)
            block_height = len(lines) * line_height + 5
            if blocks and y + used_height + block_height > bottom:
                break
            blocks.append(lines)
            used_height += block_height
            if y + used_height >= bottom:
                break
        if not blocks:
            blocks = [wrap_text("Nenhuma ação registrada ainda.", self.font_tiny, width, max_lines=2)]
        cursor_y = y
        for lines in reversed(blocks):
            for line in lines:
                if cursor_y + line_height > bottom:
                    return
                self.screen.blit(self.font_tiny.render(line, True, SUBTLE), (x, cursor_y))
                cursor_y += line_height
            cursor_y += 5

    def _history_entries(self, snapshot: dict[str, object]) -> list[str]:
        entries = snapshot.get("full_log") or snapshot.get("log") or []
        return [str(entry) for entry in entries]

    def _wrapped_history_lines(self, snapshot: dict[str, object], width: int) -> list[str]:
        entries = self._history_entries(snapshot)
        if not entries:
            return ["Nenhuma ação registrada ainda."]
        lines: list[str] = []
        for entry in entries:
            lines.extend(wrap_text(str(entry), self.font_small, width, max_lines=5))
            lines.append("")
